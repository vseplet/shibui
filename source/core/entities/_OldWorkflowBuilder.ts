...
import { _OLdTaskBuilder } from "$core/entities";

export class _OldWorkflowBuilder<ContextPot extends TPot, S extends TSpicy>
  ...
  #task1(
    builder: _OLdTaskBuilder<S, ContextPot, TPot, TPot, TPot, TPot>,
  ): _OLdTaskBuilder<S, ContextPot, TPot, TPot, TPot, TPot> {
    builder.belongsToWorkflow(this);
  ...
  sq(
    ...
    const initArgs = {
      task1: (): _OLdTaskBuilder<
        ...
        TPot
      > => {
        const builder = new _OLdTaskBuilder<
          S,
          ContextPot,
          TPot,
          TPot,
